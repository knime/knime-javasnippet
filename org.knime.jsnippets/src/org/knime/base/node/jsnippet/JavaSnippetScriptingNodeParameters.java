...
public final class JavaSnippetScriptingNodeParameters implements NodeParameters {
    ...
    public static final class InputColumnField implements NodeParameters {
        ...
        // The ValueProvider keeps this populated whenever the column selection changes (including
        // for newly added rows), so InColListPersistor.save() can always call setConverterFactory().
        @PersistArrayElement(InputColumnKnimeTypePersistor.class)
        @ValueProvider(InputColumnKnimeTypeValueProvider.class)
        ...
        /**
         * Provides the KNIME {@link DataType} of the currently selected input column. This keeps
         * {@code m_knimeType} up to date whenever the column selection changes – including for
         * newly added array rows that have no legacy NodeSettings entry.  Without this provider,
         * {@code m_knimeType} would be {@code null} for new entries and
         * {@link InColListPersistor#save} could not call
         * {@link InCol#setConverterFactory(DataType, DataCellToJavaConverterFactory)}, causing
         * the converter-factory ID to be omitted from the saved NodeSettings and a subsequent
         * {@link NullPointerException} in {@link InCol#loadSettings}.
         */
        static final class InputColumnKnimeTypeValueProvider implements StateProvider<DataType> {
            private Supplier<String> m_selectedColumnSupplier;
            ...
            @Override
            public DataType computeState(final NodeParametersInput context)
                ...
                return selectedColumnSpec.getType();
            }
        }
