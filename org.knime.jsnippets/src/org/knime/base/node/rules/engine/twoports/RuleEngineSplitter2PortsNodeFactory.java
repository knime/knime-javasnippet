...
public final class RuleEngineSplitter2PortsNodeFactory extends RuleEngineFilter2PortsNodeFactory {
    ...
    private static final String NODE_NAME = "Rule-based Row Splitter (Dictionary)";

    private static final String NODE_ICON = "./rule_engine_splitter.png";
    ...
    private static final String FULL_DESCRIPTION = """
            ...
            in the defined order. If a rule matches and the outcome is <tt>TRUE</tt>, the row will be selected for \
            inclusion in the first output table. If the first matching rule yields <tt>FALSE</tt> the row will be \
            included in the second output table. If no rule matches this row will be put to the second output table \
            (like when the last rule would be <tt>TRUE => FALSE</tt>). The roles of first and second output table can \
            be exchanged, see option below.
            <p>
...
$Col0$ > 0 => TRUE
            ...
            When the values in Col0 are greater than 0, we select the row to the first outport (if no previous rule \
            matched with FALSE outcome).
            <pre>
...
$Col0$ MATCHES $${SFlowVar0}$$ OR $$ROWINDEX$$ &lt; $${IFlowVar1}$$ =>
    ...
    private static final List<PortDescription> INPUT_PORTS = List.of( //
        fixedPort("Input table", "Any data table that should be split"), //
        fixedPort("Rules", "Table containing the rules"));

    private static final List<PortDescription> OUTPUT_PORTS = List.of( //
        fixedPort("First output", "First output table with rows that are evaluated to TRUE"), //
        fixedPort("Second output", "Second output table with rows that are evaluated to FALSE"));
    ...
    public NodeDialog createNodeDialog() {
        return new DefaultNodeDialog(SettingsType.MODEL, RuleEngineSplitter2PortsNodeParameters.class);
    }
    ...
    public NodeDescription createNodeDescription() {
        return DefaultNodeDescriptionUtil.createNodeDescription( //
            ...
            FULL_DESCRIPTION, //
            List.of(), //
            RuleEngineSplitter2PortsNodeParameters.class, //
            null, //
    ...
    public KaiNodeInterface createKaiNodeInterface() {
        return new DefaultKaiNodeInterface(Map.of(SettingsType.MODEL, RuleEngineSplitter2PortsNodeParameters.class));
    }
